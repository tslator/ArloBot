 ...
 */
static int pinWrite(int pin, int state)
{
    if (state)
        OUTA |= (1 << pin);
    else
 ...
 */
static void pinPulseHL(int pin, int d, int d1)
{
   pinHigh(pin);
   if (d > 10)
	   delay(d);
 ...
 */
static void pinPulseLH(int pin, int d, int d1)
{
   pinHigh(pin);
   if (d > 10)
	   delay(d);
